import{dj as o,dp as pe,di as T,kZ as ue,hm as J,sI as fe,kX as ge,jg as _e,eB as K,GH as me,GE as ve,H9 as Pe,EI as f,FU as w,GJ as be,EJ as z,EG as r,eT as U,vy as Q,GG as we,FC as S,sC as Ve,eC as m,i2 as $,fB as N,Ha as De,mz as Y,FS as ee,Ff as C,sG as xe,eD as v,Hb as te,lc as ie,gS as ne,it as Ee,ir as ye,hx as Le,iI as Se,i1 as Ce,dk as se,sD as Me,EK as ae,EL as re,_ as le,GN as Ae,Ey as $e,eA as H,iA as I,GF as Re,vN as Te,k9 as Oe,Fd as Ie,G0 as Ue,EF as F,Hc as qe,n as u,EZ as D,GO as je,GP as W,Hd as ze,lZ as Ne,He as R,Hf as He,iC as Fe,iE as We,u7 as Ge,iF as Be,iG as ke,iH as Ze,p as M,u as Xe,Hg as Je,mF as G}from"./index-Br3x8Ng2.js";import{t as Ke}from"./VisualElement-BG046jvK.js";let B=class{constructor(e){this._renderCoordsHelper=e,this._origin=o(),this._dirty=!1,this._count=0}set vertices(e){const n=pe(3*e.length);let i=0;for(const s of e)n[i++]=s[0],n[i++]=s[1],n[i++]=s[2];this.buffers=[n]}set buffers(e){if(this._buffers=e,this._buffers.length>0){const n=this._buffers[0],i=3*Math.floor(n.length/3/2);T(this._origin,n[i],n[i+1],n[i+2])}else T(this._origin,0,0,0);this._dirty=!0}get origin(){return this._origin}draw(e){const n=this._ensureVAO(e);n!=null&&(e.bindVAO(n),e.drawArrays(ue.TRIANGLES,0,this._count))}dispose(){this._vao!=null&&this._vao.dispose()}get _layout(){return this._renderCoordsHelper.viewingMode===1?Ye:et}_ensureVAO(e){return this._buffers==null?null:(this._vao??=this._createVAO(e,this._buffers),this._ensureVertexData(this._vao,this._buffers),this._vao)}_createVAO(e,n){const i=this._createDataBuffer(n);return this._dirty=!1,new fe(e,new ge(e,_e(this._layout),i))}_ensureVertexData(e,n){if(!this._dirty)return;const i=this._createDataBuffer(n);e.buffer()?.setData(i),this._dirty=!1}_createDataBuffer(e){const n=e.reduce((g,c)=>g+k(c),0);this._count=n;const i=this._layout.createBuffer(n),s=this._origin;let a=0,l=0;const d="startUp"in i?this._setUpVectors.bind(this,i):void 0;for(const g of e){for(let c=0;c<g.length;c+=3){const L=T(Z,g[c],g[c+1],g[c+2]);c===0?l=this._renderCoordsHelper.getAltitude(L):this._renderCoordsHelper.setAltitude(L,l);const _=a+2*c;d?.(c,_,g,L);const j=K(Z,L,s);if(c<g.length-3){for(let P=0;P<6;P++)i.start.setVec(_+P,j);i.extrude.setValues(_,0,-1),i.extrude.setValues(_+1,1,-1),i.extrude.setValues(_+2,1,1),i.extrude.setValues(_+3,0,-1),i.extrude.setValues(_+4,1,1),i.extrude.setValues(_+5,0,1)}if(c>0)for(let P=-6;P<0;P++)i.end.setVec(_+P,j)}a+=k(g)}return i.buffer}_setUpVectors(e,n,i,s,a){const l=this._renderCoordsHelper.worldUpAtPosition(a,Qe);if(n<s.length-3)for(let d=0;d<6;d++)e.startUp.setVec(i+d,l);if(n>0)for(let d=-6;d<0;d++)e.endUp.setVec(i+d,l)}};function k(t){return 3*(2*(t.length/3-1))}const Qe=o(),Z=o(),Ye=J().vec3f("start").vec3f("end").vec2f("extrude").vec3f("startUp").vec3f("endUp"),et=J().vec3f("start").vec3f("end").vec2f("extrude");function oe(t,e){const n=t.fragment;n.include(me),t.include(ve),n.include(Pe),n.uniforms.add(new f("globalAlpha",i=>i.globalAlpha),new w("glowColor",i=>i.glowColor),new f("glowWidth",(i,s)=>i.glowWidth*s.camera.pixelRatio),new f("glowFalloff",i=>i.glowFalloff),new w("innerColor",i=>i.innerColor),new f("innerWidth",(i,s)=>i.innerWidth*s.camera.pixelRatio),new be("depthMap",i=>i.depth?.attachment),new z("normalMap",i=>i.normals)),n.code.add(r`vec4 premultipliedColor(vec3 rgb, float alpha) {
